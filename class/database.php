...
abstract class database{
	...
	protected function runQuery($sql,$is_die=false){
		try {
			if ($is_die) {
				echo $sql;
				exit();
	...
	protected final function adddata($data,$is_die=false){
		...
		try {
			...
			*/
			...
			if (isset($data) && !empty($data)) {
				...
				throw new Exception("Data Can't be insert without datas");
			}
			if ($is_die) {
				debugger($data);
				debugger($this->sql,true);
	...
	protected final function deletedata($args,$is_die=false){
